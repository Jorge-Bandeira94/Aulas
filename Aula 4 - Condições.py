...
# Uma bifurcação na estrada faz com que um carro autonomo precise tomar uma decisão entre dois caminhos e dependendo do caminho escolhido, os comandos futuros serão diferentes
# Na programação o comando que diz qual bloco de comandos será utilizado depois de uma escolha é 'if'
# Nestes tipos de comandos serão utilizandos as identações para criar uma hierarquia do tipo
# A identação é executada pela TAB
'''
carro.siga()
...
# No python, esta linguagem é da seguinte forma: 'ifcarro.esquerda:' e se não: 'else:'
tempo = int(input('Quantos anos tem seu carro?'  ))
if tempo <=3:
    print('Carro novo')
else:
    print('Carro velho')
print('--Fim--') #sempre será executado, não está identado
...
# Parte prática, sem condição falsa 'else'
nome = str(input('Qual o seu nome?' )).strip().lower()
if nome == 'jorge':
    print('Que nome lindo você tem!') #Esse comando só vai acontecer se o nome foi 'Jorge', se não, só a mensagem abaixo é exibida
print('Bom dia, {}!'.format(nome.capitalize())) #Sem identação, sempre vai ocorrer

# Adicionando a condição falsa 'else'
nome = str(input('Qual o seu nome?' )).strip().lower() #coloquei lower para poder encontrar o nome de qualquer forma
